...
import { Observation } from '../schemas';
...
export class InternalService {
  ...
  async markProcedureAsSend(id_procedure: string) {
    return await this.internalProcedureModel.updateOne(
      { _id: id_procedure },
      { enviado: true },
    );
  }
  ...
  async update(id_procedure: string, procedure: UpdateInternalProcedureDto) {
    return this.internalProcedureModel
  ...
  async search(
    ...
    const [procedures, length] = await Promise.all([
      this.internalProcedureModel
        ...
        .limit(limit)
        .populate('tipo_tramite', 'nombre'),
      await this.internalProcedureModel.count({
        cuenta: id_account,
        estado: { $ne: 'ANULADO' },
        $or: [
          { alterno: regex },
  ...
  async getAllDataProcedure(id_procedure: string) {
    const procedure = await this.internalProcedureModel
  ...
  async generateAlterno(id_dependency: string, id_typeProcedure: string) {
    ...
    const correlativo = await this.internalProcedureModel.count({
      alterno: regex,
    });
    return `${typeProcedure.segmento}-${
      dependency.institucion.sigla
    }-2023-${String(correlativo + 1).padStart(5, '0')}`;
  }
}
