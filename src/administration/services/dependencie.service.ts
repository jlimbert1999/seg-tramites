...
export class DependencieService {
  ...
  async get(limit: number, offset: number) {
    ...
    const [dependencies, length] = await Promise.all([
      this.dependencyModel.find({}).populate('institucion').skip(offset).limit(limit).sort({ _id: -1 }),
  ...
  async add(dependency: CreateDependencyDto) {
    const duplicate = await this.dependencyModel.findOne({
      $or: [{ sigla: dependency.sigla }, { codigo: dependency.codigo }],
    });
  ...
  async edit(id_dependency: string, dependency: UpdateDependencyDto) {
    ...
    return await this.dependencyModel
      .findByIdAndUpdate(id_dependency, dependency, { new: true })
      .populate('institucion');
  }
  ...
  async delete(id_dependency: string) {
    const dependencyDb = await this.dependencyModel.findById(id_dependency);
    if (!dependencyDb) throw new BadRequestException('La dependencia no existe');
    const newDependency = await this.dependencyModel.findByIdAndUpdate(
      id_dependency,
      { activo: !dependencyDb.activo },
      { new: true },
    );
